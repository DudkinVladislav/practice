...
.error {
    ...
    .btn {
      ...
      <?php
      if (!empty($_POST["person_nomer"]))
        $nomer_person=$_POST["person_nomer"];
      elseif(!empty($_COOKIE["person_nomer"]))
        $nomer_person=$_COOKIE["person_nomer"];
$nomer=$_COOKIE['nomer'];
...
$stmt31 = sqlsrv_query( $conn, "SELECT nickname FROM users WHERE nomer = ?", [$nomer_person]);
$row_person = sqlsrv_fetch_array($stmt31, SQLSRV_FETCH_ASSOC);
?>
    <title>Привычки <?php echo $row_person["nickname"]; ?> </title>
  </head>
<?php
$nomer=$_COOKIE['nomer'];
setcookie('nomer', $nomer);
...
<h2> ПРИВЫЧКИ <?php echo $row_person["nickname"]; ?> </h2>
</header>
<?php
if($requests==0){
...
<table align="center" style="text-align: center; width:50%; border: 1px solid blue;">
    <?php 
    $first=1;
    $stmt31 = sqlsrv_query( $conn, "SELECT habits.habit_name, habits.habit_nomer FROM habits WHERE habits.user_nomer=? and public_habit=1", [$nomer_person]);
    ...
    <tr>
        <td>
            ...
            <input type="hidden" name="see_habit" habit_nomer="<?php echo $row31["habit_nomer"]; ?>" value="public">
...
<?php 
setcookie('person_page', $nomer_person);
?>
<br>
<br>
<a href="person_page.php"  title = "return">Вернуться на страницу пользователя</a>
<br>
<br>
